...
def check_crashed(state: BasePlaneState, agent_id):
    ...
    # # 仅在触发时打印各 mask 与关键量（不会改变逻辑）
    # def _dbg(_):
    #     return jax.debug.print(
    #         "CRASH agent={agent} masks: coll={m1} extreme={m2} highV={m3} lowV={m4} lowAlt={m5} overload={m6} highAlt={m7} | "
    #         "vt={vt:.1f} alt={alt:.1f} PQR=({P:.2f},{Q:.2f},{R:.2f}) a=({ax:.2f},{ay:.2f},{az:.2f})",
    #         agent=agent_id, m1=mask1, m2=mask2, m3=mask3, m4=mask4, m5=mask5, m6=mask6, m7=mask7,
    #         vt=state.vt[agent_id], alt=state.altitude[agent_id],
    #         P=state.P[agent_id], Q=state.Q[agent_id], R=state.R[agent_id],
    #         ax=state.ax[agent_id], ay=state.ay[agent_id], az=state.az[agent_id]
    #     )
    # def _noop(_):
    #     return None
    # jax.lax.cond(crashed, _dbg, _noop, operand=None)


    return crashed

def check_crashed_full_domain(state: BasePlaneState, agent_id,
                              min_altitude=500.0, max_overload=12.0,
                              min_velocity_mach=0.005, max_velocity_mach=3.0,
                              rotation_limit=1000.0, max_altitude=30000.0):
    """Relaxed crash check for full-domain maneuver training."""
    mask1 = check_collision(state, agent_id)
    mask2 = check_extreme_state(state, agent_id, rotation_limit=rotation_limit)
    mask3 = check_high_speed(state, agent_id, max_velocity=max_velocity_mach)
    mask4 = check_low_speed(state, agent_id, min_velocity=min_velocity_mach)
    mask5 = check_low_altitude(state, agent_id, altitude_limit=min_altitude)
    mask6 = check_overload(state, agent_id, max_overload=max_overload)
    mask7 = check_high_altitude(state, agent_id, altitude_limit=max_altitude)
    crashed = mask1 | mask2 | mask3 | mask4 | mask5 | mask6 | mask7
    return crashed
